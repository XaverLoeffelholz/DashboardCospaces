...
public class GvrReticle : MonoBehaviour, IGvrGazePointer {
  ...
  private GameObject targetObj;
	...
	public GameObject newReticleWhite;
	public GameObject newReticleBG;
  ...
  private float reticleOuterDiameter = 0.0f;
	...
	private bool hoveringInteractableObject = false;
  ...
  void Update() {
		if (walking) {
			...
			Vector3 position = transform.parent.parent.parent.localPosition;
			position = position + direction.normalized * 0.1f;
			transform.parent.parent.parent.localPosition = position; 
		}
  ...
  public void OnGazeStart(Camera camera, GameObject targetObject, Vector3 intersectionPosition,
    ...
    SetGazeTarget(intersectionPosition, isInteractive);
		...
		hoveringInteractableObject = true;

		if (newReticleWhite != null) {
			newReticleWhite.GetComponent<CustomReticle> ().Focus ();
			newReticleBG.GetComponent<CustomReticle> ().Focus ();
		}
  }
  ...
  public void OnGazeExit(Camera camera, GameObject targetObject) {
    ...
    reticleOuterAngle = kReticleMinOuterAngle;
		...
		hoveringInteractableObject = false;

		if (newReticleWhite != null) {
			newReticleWhite.GetComponent<CustomReticle> ().UnFocus ();
			newReticleBG.GetComponent<CustomReticle> ().UnFocus ();
		}
  }
  ...
  public void OnGazeTriggerStart(Camera camera) {
    // Put your reticle trigger start logic here :)
		if (newReticleWhite != null) {
			newReticleWhite.GetComponent<CustomReticle> ().Press ();
			newReticleBG.GetComponent<CustomReticle> ().Press ();
		}

		if (!hoveringInteractableObject) {
			// Walking
			walking = true;
		}
  }
  ...
  public void OnGazeTriggerEnd(Camera camera) {
    // Put your reticle trigger end logic here :)
		...
		if (newReticleWhite != null) {
			newReticleWhite.GetComponent<CustomReticle> ().Release ();
			newReticleBG.GetComponent<CustomReticle> ().Release ();
		}
